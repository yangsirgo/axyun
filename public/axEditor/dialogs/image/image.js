...
(function () {
    ...
    function initScroll(){
        ...
        $('#imageList').scroll(function (){
            var h = $(this).height();
            var ch = $(this).find('.list').height();
            var scrollTop = $(this).scrollTop();
            if (h + scrollTop  >= ch - 10 ) {
                if (timerObj != null) return;
                timerObj = setTimeout(function (){
                    pageNo++;
                    onlineImage.isLoadingData = false;
                    onlineImage.getImageData(pageNo)
    ...
    RemoteImage.prototype = {
        ...
        setImage: function(img){
            ...
            if(src) {
                ...
                $G("title").value = img.title.replace(reg, "") || img.alt.replace(reg, "") || img.original.replace(reg, "") || '';
                setAlign(align);
                this.setPreview();
                this.updateLocker();
            }
    ...
    OnlineImage.prototype = {
        ...
        getImageData: function (page) {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url = editor.getActionUrl('apiUrl') + '/image/getByParamsInPage',
                ...
                ajax.request(url, {
                    ...
                    headers: {
                        ...
                        '_hosId': userInfo._hosId,
                        '_hosNm': userInfo._hosNm,
                        '_hosCd': userInfo._hosCd,
                        '_orgId': userInfo._orgId,
                        '_orgNm': userInfo._orgNm,
                        '_orgCd': userInfo._orgCd,
                        '_currDept': userInfo._currDept,
                        '_currRole': userInfo._currRole,
                        '_currDeptNm': userInfo._currDeptNm,
                        '_currDeptCd': userInfo._currDeptCd,
                        '_workType': userInfo._workType,
                        '_roleType': userInfo._roleType
                    },
                    'method': 'get',
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].fileUrl) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', (list[i].masterUrl.indexOf('http://') > -1?list[i].masterUrl: 'http://' + list[i].masterUrl) + list[i].imageUrlId + (list[i].fileUrl.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', (list[i].masterUrl.indexOf('http://') > -1?list[i].masterUrl: 'http://' + list[i].masterUrl) + list[i].imageUrlId);
                    img.setAttribute('_name', list[i].imageName);
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    var img = lis[i].firstChild,
                        ...
                        name = img.getAttribute('_name');
                    list.push({
                        src: src,
                        _src: src,
                        alt: name || src.substr(src.lastIndexOf('/') + 1),
                        floatStyle: align
                    });
                }
