(function (AX) {
  ...
  window.onload = function () {
    ...
    if (AX.plugins[thePlugins].editdom) {
      ...
      // $.each(attrs, function (index, item) {
      //   if (item.teeth == 1) {
      //     $('.teethItem' + index).addClass('selected');
      //     if (item.yamian != []) {
      //       var ymVal = [];
      //       $('.yamianTeethItem' + index).removeClass('selected');
      //       $.each(item.yamian, function (key, value) {
      //         $('.yamianTeethItem' + index + '.yamianTeeth' + value).addClass('selected');
      //         ymVal.push(ymValue[parseFloat(value)]);
      //       });
      //       $('.teethItemI' + index).html(ymVal.join(''));
      //     } else {
      //       $('.yamianTeethItem' + index).removeClass('selected');
      //       $('.teethItemImg.teethItemImg' + index + ' .teethItemI').empty();
      //     }
      //   } else {
      //     $('.teethItem' + index).removeClass('selected');
      //     $('.yamianTeethItem' + index).removeClass('selected');
      //     $('.teethItemImg.teethItemImg' + index).html((index < 16 ? '<div class="h-line"></div>' : '') + (index == 7 || index == 23 ? '<div class="v-line"></div>' : ''));
      //   }
      //   keepWidthSame(index)
      // })
      init()
  ...
  $.each($('.topTeeth .teethItem'), function (index, item) {
    dom += '<li class="topYaMianTeeth' + (index == 7 ? ' mr' : '') + '" teeth="' + index + '">' +
  ...
  $.each($('.bottomTeeth .teethItem'), function (index, item) {
    ...
    bottomDom += '<li class="bottomYaMianTeeth' + (index == 7 ? ' mr' : '') + '" teeth="' + index + '">' +
  ...
  $('.teethItem').click(function () {
    ...
    if (!$(this).hasClass('selected')) {
      $(this).addClass('selected');
      $('.teethItemImg' + teethIndex).html((teethIndex < 16 ? '<div class="h-line"></div>' : '') + (teethIndex == 7 || teethIndex == 23 ? '<div class="v-line"></div>' : '') + '<span class="teethItemText">' + teethContent + '</span>' + '<i class="teethItemI teethItemI' + teethIndex + '" teeth="' + teethIndex + '"></i>');
      attrs[teethIndex].teeth = 1;
    } else {
      $(this).removeClass('selected');
