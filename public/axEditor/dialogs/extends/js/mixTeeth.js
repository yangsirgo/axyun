(function (AX) {
  ...
  function init(previewDom) {
    $.each(attrs, function (index, item) {
      if (item.teeth == 1) {
        ...
        if (item.teethType == 1) {
          //恒牙
          $('.teethHeng .teethItem' + index, previewDom).addClass('selected');
          $('.teethRu .teethItem' + index, previewDom).removeClass('selected');
          $('.teethItemImg' + index, previewDom).html((index < 16 ? '<div class="h-line"></div>' : '') + ((index == 7 || index == 23) ? '<div class="v-line"></div>' : '') + '<span class="teethItemText" style="visibility:hidden">'+ $('.teethHeng .teethItem' + index).html() +'</span>');
        } else {
          //乳牙
          $('.teethRu .teethItem' + index, previewDom).addClass('selected');
          $('.teethHeng .teethItem' + index, previewDom).removeClass('selected');
          $('.teethItemImg' + index, previewDom).html((index < 16 ? '<div class="h-line"></div>' : '') + ((index == 7 || index == 23) ? '<div class="v-line"></div>' : '') + '<span class="teethItemText" style="visibility:hidden">'+ $('.teethRu .teethItem' + index).html() +'</span>');
        }
        /*判断是否有牙面*/
        if (item.yamian != []) {
          var ymVal = [];
          ...
          $('.teethItemImg' + index, previewDom).append('<i class="teethItemI teethItemI' + index + '" teethType="1" teeth="1">' + ymVal.join('') + '</i>');
        } else {
          $('.yamianTeethItem' + index, previewDom).removeClass('selected');
          $('.teethItemImg.teethItemImg' + index + ' .teethItemI', previewDom).html((index < 16 ? ((index == 7 || index == 23) ? '<div class="h-line"></div><div class="v-line"></div>' : '<div class="h-line"></div>') : ''));
        }
      } else {
        $('.teethItem' + index, previewDom).removeClass('selected');
        $('.yamianTeethItem' + index, previewDom).removeClass('selected');
        $('.teethItemImg.teethItemImg' + index, previewDom).html((index < 16 ? ((index == 7 || index == 23) ? '<div class="h-line"></div><div class="v-line"></div>' : '<div class="h-line"></div>') : ''));
      }
      keepWidthSame(index, null);
  ...
  window.onload = function () {
    ...
    if (AX.plugins[thePlugins].editdom) {
      ...
      //默认恒牙，牙面全不选中状态
      $('.teethHeng .teethItem').addClass('selected');
  ...
  var dom = '';
  var bottomDom = '';
  $.each($('.topTeethHeng .teethItem'), function (index, item) {
    dom += '<li class="topYaMianTeeth" teeth="' + index + '">' +
  ...
  $.each($('.bottomTeethHeng .teethItem'), function (index, item) {

    bottomDom += '<li class="bottomYaMianTeeth" teeth="' + index + '">' +
  ...
  $('.yamianTeethItem').click(function () {
    ...
    if (!$(this).hasClass('selected')) {
      ...
      if ($('.teethItem' + teethIndex).hasClass('selected')) {
        ...
        attrs[teethIndex].yamianHtml.push(yamian);
      } else {
        //之前牙齿未选中状态
        $('.teethItem' + teethIndex).addClass('selected');
        $('.teethItemImg' + teethIndex).html((teethIndex < 16 ? '<div class="h-line"></div>' : '') + ((teethIndex == 7 || teethIndex == 23) ? '<div class="v-line"></div>' : '') + '<span class="teethItemText">' + teethContent + '</span>' + '<i class="teethItemI teethItemI' + teethIndex + '" teeth="' + teethIndex + '">' + yamian + '</i>');
        attrs[teethIndex].yamian.push(yamianIndex);
        attrs[teethIndex].yamianHtml.push(yamian);
      }
    } else {
      ...
      attrs[teethIndex].yamian.splice(attrs[teethIndex].yamian.indexOf(yamianIndex), 1);
      attrs[teethIndex].yamianHtml.splice(attrs[teethIndex].yamianHtml.indexOf(yamian), 1);
    }
  ...
  function keepWidthSame(teethIndex,type, previewDom){
    ...
    reflectObj.css({'padding' : '0 ' + (reflectObj.attr('teethType') == 0 ? 3 : 6) + 'px'});
    currObj.attr('teethType',type).css({'padding' : '0 ' + (type == 0 ? 3 : 6) + 'px'});
  ...
  $('.teethItem').click(function () {
    ...
    if (!$(this).hasClass('selected')) {
      /*之前未选中*/
      //乳牙恒牙切换
      if ($(this).attr('teethType') == '1') {
        $('.teethRu .teethItem' + teethIndex).removeClass('selected');
        attrs[teethIndex].teethType = 1;
      } else {
        $('.teethHeng .teethItem' + teethIndex).removeClass('selected');
        attrs[teethIndex].teethType = 0;
      }
      $(this).addClass('selected');

      $('.teethItemImg' + teethIndex).html((teethIndex < 16 ? '<div class="h-line"></div>' : '') + ((teethIndex == 7 || teethIndex == 23) ? '<div class="v-line"></div>' : '') + '<span class="teethItemText">' + teethContent + '</span>' + '<i class="teethItemI teethItemI' + teethIndex + '" teeth="' + teethIndex + '">' + attrs[teethIndex].yamianHtml.join('') + '</i>');
      attrs[teethIndex].teeth = 1;
    } else {
      $(this).removeClass('selected');
      ...
      attrs[teethIndex].yamianHtml = [];
    }
    keepWidthSame(teethIndex,$(this).attr('teethType'));
  });
