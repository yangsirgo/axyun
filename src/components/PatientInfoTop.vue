<template>
  <el-card class="width100 card-cont">
    <div class="top-cont width100 height100">
      <div class="left">
        <img src="@/assets/pojehjdz.jpeg" alt=""/>
      </div>
      <div class="center">
        <el-row class="patient-name">{{ patientInfo.patientName || '--' }}</el-row>
        <el-row class="patient-gender" :gutter="10">
          <el-col :span="16"><span :val="patientInfo.patientGender || ''" code="GENDER_CODE" v-codeTransform></span></el-col>
          <el-col :span="8">{{ patientInfo.patientAge  || '--'}}</el-col>
        </el-row>
      </div>
      <div class="right">
        <el-row class="patient-top">
          <el-col :span="8">出生日期<i>{{patientInfo.birthDate || '--'}}&nbsp;&nbsp;{{patientInfo.birthTime}}</i></el-col>
          <el-col :span="8" v-if="patientInfo.identificationType != undefined && patientInfo.identificationType != ''"><span :val="patientInfo.identificationType || ''" code="MARK_TYPE_CODE" v-codeTransform></span><i>{{patientInfo.identificationNum || '--'}}</i></el-col>
          <el-col :span="8">籍贯<i code="AdministrativDivision" :val="patientInfo.nativePlaceCode || ''" break="/" v-codeTransform></i></el-col>
        </el-row>
        <el-row class="patient-bottom">
          <el-col :span="8">手机号码<i>{{patientInfo.mobileNum || '--'}}</i></el-col>
          <!-- <el-col :span="8">医保类型<i :val="patientInfo.ssType || ''" code="socialSecurity" v-codeTransform></i></el-col> -->
        </el-row>
      </div>
      <div class="right1 float-right">
        <div class="left-info float-left">
          <el-row class="empiid">患者编号 <i>{{ patientInfo.patientCode  || '--'}}</i></el-row>
