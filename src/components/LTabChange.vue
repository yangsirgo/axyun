    ...
    updated (){
      ...
      document.onkeydown = function (e){ // isEdit 为false的时候 禁用tab
        if(!_this.isEdit && e && e.keyCode == 9){
          if(document.activeElement.getAttribute('tablast') == 1){
      ...
      document.onkeyup = function (e) {
        ...
        if(e && e.keyCode == 9){ // 按 Esc
          if(_this.isEdit){
            if(document.activeElement.getAttribute('tablast') == 1){ // 判断当前光标所在元素是否为可编辑最后一个
              _this.isEdit = false
              if (e.preventDefault) {
                e.preventDefault();
              }else {
    ...
    watch: {
      tableData:{ // 监听表格数据
        ...
        handler(val){
          ...
          setTimeout(function () {
            val.map((item, index) => {
              if(item.isEditing && document.activeElement.getAttribute('tablast') != 1){
                _this.isEdit = true;
                const row = document.getElementById(_this.id).querySelectorAll('.el-table__row')[index];
                const input = row.querySelectorAll('input');
                row.querySelectorAll('input')[row.querySelectorAll('input').length - 1].setAttribute('tablast', 1)
