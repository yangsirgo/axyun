...
// 调用横向分页
export function reReuse(domValue, domSS) {
  if (!domSS) {
    domSS = domValue;
  }
  let domS = domSS.domStr;
  let domV = domSS;
  let str = reRatas(domSS);
...
export function reRatas(domResult, typeData) {
  ...
  for (let j = 0; j < capName.length; j++) {
    ...
    console.log('capName[j]', capName[j].parentElement.innerHTML)
  ...
  for (let k = 0; k < titleList.length; k++) {
    content = getTable(doc, titleList[k], domResult);
    doc.innerHTML = content;
  }
  /**
   * 空白替代标签/或者内容替代标签
  ...
  if (barList.length > 0) {
    for (let bar = 0; bar < barList.length; bar++) {
      if (!domResult[barList[bar].toString()]) {
        content = content.replace(STR_Bar + barList[bar].toString() + STR_M_E, "");
      } else {
        content = content.replace(STR_Bar + barList[bar].toString() + STR_M_E, "<canvas id='" + barList[bar].toString() + "'></canvas>");
...
export function getTable(doc, tableName, dataR, typeData) {
  ...
  for (let k = 0; k < table.length; k++) {
    if (table[k].getAttribute("id") == tableName) {
      ...
      let cfLength = 0;
      // debugger
      for (let cf = 0; cf < trs.length; cf++) {
        var tds = trs[cf].querySelectorAll(STR_TD);
        if (tds.length > 0) {
          cfStr = trs[cf].outerHTML;
          if (cfStr.indexOf(STR_CF) >= 0 || cfStr.indexOf(STR_CP) >= 0 || cfStr.indexOf(STR_CM) >= 0) {
            indexCf = cf;
            if (cfStr.indexOf(STR_CL) >= 0) {
              let leg = getTableName(cfStr, STR_CL);
              cfLength = leg[0].slice(1);
              cfStr = cfStr.replace(STR_CL + cfLength + STR_M_E, "")
            }
            break;
          }
        }
      }
      let cfList = getTableName(cfStr, STR_CF);
      let cpList = getTableName(cfStr, STR_CP);
      let cmList = getTableName(cfStr, STR_CM);
      let length = 0;
      // debugger
      if (!dataList) {
        dataList = [];
      ...
      if (cfLength) {
        if (cfLength > trs.length || (cfLength < trs.length && cfLength >= indexCf + dataList.length)
          || !cfLength) {
          if ((trs.length >= indexCf + dataList.length)) {
            length = indexCf + dataList.length;
          } else {
            length = trs.length;
          }
        } else if (cfLength < trs.length && cfLength < indexCf + dataList.length) {
          length = JSON.parse(cfLength) + indexCf;
        }
      } else {
        if (cfList.length || cpList.length || cmList.length) {
          if (trs.length >= indexCf + dataList.length) {
            console.log('indexCf,dataList.length', indexCf, dataList.length)
            length = indexCf + dataList.length;
          } else {
            length = trs.length;
          }
      ...
      if (length-indexCf > 0) {
        for (let c = indexCf; c < length; c++) {
          let oldTrsStrs = trs[c].outerHTML;
          let val = cfStr;
          if (cfList.length > 0) {
            for (let cv = 0; cv < cfList.length; cv++) {
              if (cfList[cv].indexOf('-') == -1) {
                console.log('c - indexCf', dataList[c - indexCf])

                if (dataList[c - indexCf] == undefined || dataList[c - indexCf][cfList[cv].slice(1)] == undefined) {
                  val = val.replace(STR_CF + cfList[cv].slice(1) + STR_M_E, "")
                } else {
                  let value = dataList[c - indexCf][cfList[cv].slice(1)] || "";
                  val = val.replace(STR_CF + cfList[cv].slice(1) + STR_M_E, value)
                }

              } else {
                var value = [];
                var vall = '';
                var arrFa = cfList[cv].slice(1).split("-");
                var list = dataList[c - indexCf][arrFa[0]];
                if (list == undefined) {
                  vall = ''
                } else {
                  for (var fa = 0; fa < list.length; fa++) {
                    if (list[fa] == undefined) {
                      ...
                      var height = parseFloat(arrFa[3]) || 30;
                      ...
                      if (fa == 0) {
                        if (arrFa.indexOf('t') >= 0) {
                          vall += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                        } else {
                          vall += "<tr><td height=" + height + "px' style='width:100%;" + align + "'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                        }
                      } else if (fa == list.length - 1) {
                        if (arrFa.indexOf('b') >= 0) {
                          vall += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-bottom:" + border + "px solid #000;border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                        } else {
                          vall += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                        }
                      } else {
                ...
                var vals = '<table style="width:100%"  border="0">' + vall + '</table>'

                val = val.replace(STR_CF + cfList[cv].slice(1) + STR_M_E, vals)
          ...
          if (cpList.length > 0) {
            for (let cp = 0; cp < cpList.length; cp++) {
              let value = "";
              let arrCondition = cpList[cp].slice(1).split(",")
              if (dataList[c - indexCf][arrCondition[0]] == undefined) {
                value = "";
              } else {
                let cof = dataList[c - indexCf];
                let str = eval(arrCondition[arrCondition.length - 1]);
                value = str;
              }

              val = val.replace(STR_CP + cpList[cp].slice(1) + STR_M_E, value);
            }
          }
          if (cmList.length > 0) {
            for (let cm = 0; cm < cmList.length; cm++) {
              let value = "";
              if (dataList[c - indexCf] == undefined || dataList[c - indexCf][cmList[cm].slice(1)] == undefined) {
                value = ""
              } else {
                value = "<img src='" + dataList[c - indexCf][cmList[cm].slice(1)] + "' style='width:80%;heigth:80%;margin-top:10%;margin-left:10%'></img>"
              }
              val = val.replace(STR_CM + cmList[cm].slice(1) + STR_M_E, value);

            }
          }

          content = content.replace(oldTrsStrs, val);
        }
      } else {
        // debugger
        for (let cm = 0; cm < cmList.length; cm++) {
          content = content.replace(STR_CM + cmList[cm].slice(1) + STR_M_E, "");

        }
        for (let cp = 0; cp < cpList.length; cp++) {
          content = content.replace(STR_CP + cpList[cp].slice(1) + STR_M_E, "");
        }
        for (let cf = 0; cf < cfList.length; cf++) {

          content = content.replace(STR_CF + cfList[cf].slice(1) + STR_M_E, "");
        }
      }
          ...
          if (list == undefined) {
            ...
            for (var fa = 0; fa < list.length; fa++) {
              if (list[fa] == undefined) {
                ...
                if (fa == 0) {
                  if (arrFa.indexOf('t') >= 0) {
                    val += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                  } else {
                    val += "<tr><td height=" + height + "px' style='width:100%;" + align + "'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                  }
                } else if (fa == list.length - 1) {
                  if (arrFa.indexOf('b') >= 0) {
                    val += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-bottom:" + border + "px solid #000;border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                  } else {
                    val += "<tr><td height=" + height + "px' style='width:100%;" + align + "border-top:" + border + "px solid #000'>" + list[fa][arrFa[1]] + '<br></td></tr>';
                  }
                } else {
  ...
  // console.log("newTrsStrnewTrsStrnewTrsStrnewTrsStr", newTrsStr)
...
export function handleStr(dataResult, dataR, allData) {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    // debugger
    if (ids[i] == barList[index]) {
      // console.log('dataR[barList[index]]##', dataR[barList[index]])
      JsBarcode(canvass[0], dataR[barList[index]], {
        text: "",
        format: "CODE39",
        displayValue: false,
        width: 1.0,
