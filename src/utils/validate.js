...
// 验证英文字母
let LetterValidation = (rule, value, callback) => {
...
// 验证身份证号
let validCnIdNo = (rule, value, callback) => {
  if (value == "" || value == undefined) {
    callback();
  } else {
    if (!testCnIdNo(value)) {
      callback(new Error("您输入的身份证号不符合规则！"));
    } else {
      callback();
    }
  }
...
export { LetterValidation, validCnIdNo, testCnIdNo };
