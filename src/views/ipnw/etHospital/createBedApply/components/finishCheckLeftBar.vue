                ...
                >
                  ...
                  <template #bottom>
                    <div class="show-patient-detail">
                      <span>身份证</span>
                      <span class="margin-left-4">{{item.identificationNum ? item.identificationNum : "--"}}</span>
                    </div>
                    <div class="show-patient-detail">
                      <span>申请时间</span>
                      <span class="margin-left-4">{{item.applyDate && item.applyDate.split(" ").length > 1 ? item.applyDate.split(" ")[0] : "--"}}</span>
                    </div>
                  </template> -->
                  <template #top>
                    <span
                  ...
                  <template #bottom>
                    <div style="font-size: 12px; color: #999; line-height: 1.5">
                      ...
                      <div>
                        申请状态：{{ filterStatusName(item.applyStatus) }}
                      </div>
...
export default {
  ...
  methods: {
    ...
    filterStatusName(status) {
      //1:提交申请 2:入院登记 3:排床 4:病区出院 5:财务出院  9:撤销 99:作废
      if (status == "1") {
        return "已申请";
      } else if (status == "2") {
        return "已登记";
      ...
      createBedHistoryList(params, false)
        .then(res => {
          ...
          let targetRow = resultData.find(item => {
            return item.patientId && item.patientId === patientId;
          });
          console.log("targetRow:::", targetRow);
          if (targetRow && this.activeName === "second") {
            // 如果列表中还有这个患者，就选中他
          ...
          } else if (this.currentPatientsList.length && this.activeName === "second"){
