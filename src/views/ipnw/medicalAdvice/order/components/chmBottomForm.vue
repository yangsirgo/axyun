...
import { saveHerbalOrder } from "@/api/cis/order/order";
...
export default {
  name: "chmBottomForm",
  props: ["tableList", "advicePO", "currentChuFangRow"],
  data() {
    return {
      chmRules: {
        chmAgent: [
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!this.currentChuFangRow.recipeCode) {
      ...
      this.chmForm.recipeId = currentChuFangRow.recipeId;

      this.$refs["chmForm"].validate(valid => {
        if (valid) {
          let self = this;
          this.$emit("saveChmTable", this.chmForm, function() {
            console.log("清空表单");
            self.reset();
          });
        }
