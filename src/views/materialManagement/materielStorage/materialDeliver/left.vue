<template>
  <el-card class="height100">
    ...
    <el-row style="padding:0 8px;margin-bottom:8px">
      <el-col>
        <l-formt-title label="出库库房" labelWidth="64" disabled>
    ...
    <!--排序-->
    <div class="sorttitle">
      <l-sort-label class="inline-block" label="出库单号" state="2" @click="listsort('INBOUND_NUM')"></l-sort-label>
      ...
      <div class="list-container overflow-scroll-vertical"  >
        <div :class="curSelectedIndex == i ? 'list-item active' : 'list-item'" v-for="(item, i) in wareList" :key="i"
          ...
          <el-row class="item-label">
            <el-col :span="24">
              <span class="rep">{{ item.outboundNum }}</span>
            </el-col>
          </el-row>
          ...
          <el-row class="item-label">
            <el-col :span="10">
              <span class="label margin-right-6">领用科室:</span>
      ...
      WarehousingAPI
        ...
        .then(res => {
          ...
          if (res.code == 1) {
            ...
            this.$message.error(res.msg || "获取出库单数据失败");
          }
          this.pageParams = res.pageParams;
        })
        .catch(err => {
        ...
        confirm: (action, instance, done) => {
          ...
          WarehousingAPI
            ...
            .catch(err => {
              ...
              this.$hideLoading();
              this.$message.error(err.msg || "删除出库单失败");
            });
        }
      });
