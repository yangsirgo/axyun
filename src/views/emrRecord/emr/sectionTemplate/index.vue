...
import {
  ...
  copyTemplates,
  getNextSeqNo
} from "@/api/emr/sectionTemplate";
import { getOrgDeptList } from "@/api/admin/user";
import { getTypes } from "@/api/emr/type";
import {
  getParagraphTypeOpt,
  getTypeByCode,
  getIcd10DictionaryByKey
} from "@/api/emr/paragraphType";
...
export default {
  ...
  methods: {
    saveCareContent() {
      ...
      this.saveContent(this.careContent, item.dnFlag);
      // this.hiddenTemplate();
      // this.careContent = "";
    ...
    //打开
    async openV() {
      console.log("openV", this.selection);
      ...
      // this.showEditorFlag = this.selection[0].dnFlag === "1" ? true : false;
        ...
        .then(res => {
          ...
          if (res.code === 1) {
            ...
            if (editor === "01") {
              ...
              // text 护理文书格式暂时不实现
              this.defaultMsg = "";
              console.log("this.sTemplate.editor====", this.sTemplate.editor);
              console.log(
                "this.sTemplateOldEditor====",
                this.sTemplateOldEditor
              );
    ...
    async saveContent(content, dnFlag) {
      let data = {
        id: this.openId,
        content: content,
        dnFlag: dnFlag
    ...
    propOnClick() {
      ...
      getDetail(this.selection[0].sectionTemplateId, data).then(res => {
        if (res.code === 1) {
          ...
          console.log("sTemplateOldEditor=====", this.sTemplateOldEditor);
          setTimeout(() => {
            this.dialogVisible = true;
          }, 200);
        } else {
    ...
    handleUpdate(template) {
      ...
      console.log("this.sTemplateOldEditor==", this.sTemplateOldEditor);
      console.log("this.sTemplate==", template);
