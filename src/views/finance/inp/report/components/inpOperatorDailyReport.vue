...
export default {
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        if (res.code == 1) {
          ...
          printData.title = "住院收费员日报表";
          let obj = await onPreviewStr(printData, "住院收费员日报表");
          this.conts = obj.str;
        } else {
    ...
    handleData(obj) {
      ...
      objStr.chargeUserNames = obj.chargeUserNames || this.chargeUserName;
      ...
      objStr.RMB = this.changeRMB(obj.totalPayAmt) || "零";
      objStr.totalFeeAmt = this.outputmoney(obj.totalFeeAmt);
      objStr.totalPayAmt = this.outputmoney(obj.totalPayAmt);
      let chargeDetailSumInfos = obj.chargeDetailSumInfos;
      objStr.payName0 = "";
      objStr.payName1 = "";
      objStr.payName2 = "";
      objStr.payName3 = "";
      ...
      for (let i = 0; i < chargeDetailSumInfos.length; i++) {
        let name = chargeDetailSumInfos[i].settleAmtName;
        let totalAmt = this.outputmoney(chargeDetailSumInfos[i].totalAmt);
      ...
      let settleAmountInfos = obj.settleAmountInfos;
      ...
      for (let i = 0; i < settleAmountInfos.length; i++) {
        let code = settleAmountInfos[i].payTypeCode;
        let payAmount = this.outputmoney(settleAmountInfos[i].payAmount);
        switch (code) {
          case "wxpay_barcode":
