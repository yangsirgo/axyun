...
<script>
import service from "@/api/financeManagement/prestoreAccount";
export default {
  components: {},
  data() {
    return {
  ...
  mounted() {
    this.getListPagePrestoreAccount();
  },
  methods: {
    getReadCard(data) {
      //获取读卡信息
    ...
    async getListPagePrestoreAccount() {
      this.loading = true;
      let res = await service.getListPagePrestoreAccount({
      ...
      this.pageParams.total = res.total;
       this.tableData =  res.data.map((item) => {
                    ...
                    item.presentPrestoreMoney = item.presentPrestoreMoney.toFixed(2);
                    item.presentMoney = item.presentMoney.toFixed(2);
                    item.consumeMoney = item.consumeMoney.toFixed(2);
                    item.grantOverdrawMoney = item.grantOverdrawMoney.toFixed(2);
