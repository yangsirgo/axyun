...
<template>
    <div class="container">
        <div class="header-button">
            <el-button  type="primary" size="mini" @click="handleAdd">添加资源</el-button>
        ...
        <div class="selectType">
            ...
            编码: <el-input  style="width: 180px" v-model="params.code" v-on:keyup.enter.native="queryHandle"/> 名称: <el-input style="width: 180px" v-model="params.name" v-on:keyup.enter.native="queryHandle"/>
        </div>
        <el-table
                :data="tableData"
        ...
        <el-dialog :visible.sync="dialogVisible1" :title="dialogType==='edit'?'编辑资源':'新建资源'" :before-close="handleClose1" >
            <el-form :model="resource" label-width="80px" label-position="right" ref="rform" :rules="rules" style="height:360px;overflow-y:auto">
...
<script>
  import {getResourceAll,addRes,addResNew,getResRouter,addResource,editResource,removeResource} from '@/api/admin/resource'
  ...
  export default {
    ...
    methods: {
      ...
      getResR() {
        getResRouter({authId: this.authId})
          .then(res => {
            let arr = []
            if (res.code === 1) {
