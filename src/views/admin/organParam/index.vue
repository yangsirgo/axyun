<template>
    <div class="container" ref="page">
        ...
        >
            ...
            <el-table-column fixed="right" label="操作">
                <template slot-scope="scope">
                    <el-button @click.native.prevent="handleCh(scope.row)" type="text" size="small" v-if="scope.row.exts.required !=='1'">改变父级</el-button>
                    <el-button @click.native.prevent="handleClick(scope.row)" type="text" size="small" v-if="scope.row.exts.required !=='1'">添加下一级</el-button>
                    <el-button @click.native.prevent="handleEdit(scope.row)" type="text" size="small" v-if="scope.row.exts.required !=='1'">编辑</el-button>

                    <el-button @click.native.prevent="handleRemove(scope.row)" type="text" size="small" v-if="scope.row.exts.required !=='1'">删除</el-button>
                </template>
            </el-table-column>
        </el-table>

        <!-- 可以拆成组件 -->
        <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑类型':'新建类型'" :before-close="handleClose" :close-on-click-modal="false">
            <el-form :model="type" label-width="80px" label-position="right" ref="type" :rules="rules" >
        ...
        <el-dialog :visible.sync="changeVisible" title="改变父级" :before-close="handleClose1" :close-on-click-modal="false">
            <el-form :model="chType" label-width="80px" label-position="left" ref="chType" :rules="rules" >
              ...
              this.$refs['type'].validate((valid) => {
                if(!valid) return;
	                if (this.type.hasOwnProperty("id")) {
	                    ...
	                    if (this.parentId) {
	                        this.addTree({ ...{ parentId: this.parentId }, ...this.type,...{paramType: this.paramType} });
	                    } else {
                    ...
                    putParent(obj)
                        .then(res=>{
                            if(res.code === 1){
                                ...
                                console.log(this.maps.has(this.chObj.parentId))
                                if(this.maps.has(this.chObj.parentId)){
                                    const {tree,treeNode,resolve} = this.maps.get(this.chObj.parentId)
                                    this.loadDel(tree,treeNode,resolve)
                                }
                                this.$nextTick(()=>{
                                    this.getTree()
                    ...
                    putParent(obj)
                        .then(res=>{
                            if(res.code === 1){
                                ...
                                this.$message('修改父级成功')
                                console.log(this.maps.has(this.chObj.id))
                                if(res.hasOwnProperty('data')){
                                    if(this.maps.has(res.data)){
                                        const {tree,treeNode,resolve} = this.maps.get(res.data)
                                        this.load(tree,treeNode,resolve)
