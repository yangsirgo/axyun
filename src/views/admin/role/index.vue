        ...
        <el-col :span="8">
          <div class="tree_box">
            <div class="tree_box_top">
              ...
              >
              <el-button type="primary" size="mini" @click="confirmRoleToAuth"
                >保存</el-button
              >
            </div>
            <el-input
            ...
            >
              ...
              >
                <div
                  ...
                  @click.stop="handleChangeHome(data, node)"
                >
                  <el-radio
                    v-if="typeof data.id != 'string' && node.checked"
                    v-model="select"
                    :label="data.id"
                    border
                    >首页</el-radio
                ...
                <div class="oneLevel" v-if="node.level == 1"></div>
                <span>{{ node.label }}</span>
                <span style="margin-left: 10px">
                  <div class="showBtn" v-if="typeof data.id === 'string'">
                    {{ typeof data.id === "string" ? "功能" : "" }}
                  </div>
                </span>
              </div>
            </el-tree>
...
export default {
  ...
  components: {
    lTable: () => import("./components/LTable.vue"),
    LTableForm: () => import("./components/LTableForm.vue"), //,
    //  lDrawer
  },
  data() {
  ...
  methods: {
    ...
    // // 选择首页
    handleChangeHome(data, node) {
      console.log("datadatadatadata", data, node);
      this.uri = data.exts.uri;
      this.aid = data.id;
    },
    ...
    handleCheckChange(data) {
      ...
      if (ids.indexOf(data.id) != -1) {
        if (data.parentId) {
          if (ids.indexOf(data.parentId) == -1) {
            
            ids.push(data.parentId);
            if(data.parentId){
              this.handlePartent(data.parentId,ids)
            }
        ...
        if (data.children) {
          this.handleChildren(data.children, "has", ids);
        }
      } else {
        if (data.children) {
          this.handleChildren(data.children, "no", ids);
        }
      }
    },
    handleChildren(data, type, ids) {
      if (type == "has") {
        data.map((val) => {
          ...
          if (val.children) {
            this.handleChildren(val.children, "has", ids);
          }
        });
      } else {
        data.map((value) => {
          ...
          if (value.children) {
            this.handleChildren(value.children, "no", ids);
          }
        });
      }
      this.$refs.tree.setCheckedKeys(ids);
    ...
    handlePartent(id,ids){
     let node = this.$refs.tree.getNode(id);
      if(node.data.parentId){
        ids.push(node.data.parentId);
        this.handlePartent(node.data.parentId,ids)
      }
        this.$refs.tree.setCheckedKeys(ids)
    },
    ...
    // 角色关联权限确认按钮
    async confirmRoleToAuth() {
      let params_ = {
        rid: this.role_id,
        aids: this.$refs.tree.getCheckedKeys(),
        aid: this.aid,
      };
      let isSelect = params_.aids.includes(this.aid);
      if (!isSelect) {
        this.aid = null;
        this.select = null;
      }
...
/deep/.el-radio--medium.is-bordered.is-checked {
