            ...
            <el-tree :expand-on-click-node="false" :data="data" default-expand-all node-key="id" ref="tree" highlight-current :filter-node-method="filterNode" :props="defaultProps" class="el-tree">
              <span class="custom-tree-node" slot-scope="{ node, data }">
                ...
                <span>
                  ...
                  <el-button type="text" size="mini" v-show="data.hosId==''||data.hosId==null||typeof(data.hosId)=='undefined'?true:false" @click="() => add(data)">添加下一级</el-button>
                  <el-button type="text" size="mini" v-show="data.hosId==''||data.hosId==null||typeof(data.hosId)=='undefined'?false:true" @click="() => modify(data)">编辑</el-button>
                  <el-button type="text" size="mini" v-show="data.hosId==''||data.hosId==null||typeof(data.hosId)=='undefined'?false:true" @click="() => remove(node,data)">删除</el-button>
                  <el-button type="text" size="mini" v-show="data.hosId==''||data.hosId==null||typeof(data.hosId)=='undefined'?false:true" @click="() => disable(data)">{{data.status=='1' ? '已停用':'停用'}}</el-button>
                </span>
              </span>
            </el-tree>
          </el-scrollbar>
        ...
        <div class="table_box" style="height:600px!important;overflow-y:auto">
          <div v-show="countyFlag">
            ...
            <div style="margin-top: 20px">
              ...
              <div class="right">
                <el-date-picker v-model="SysHosRegionVO.year" type="date" :value-format="year" :format="year1" placeholder="选择日期" :type="year2" @change="changeRegion">
                </el-date-picker>
            ...
            <div style="margin-top: 20px">
              ...
              <div class="right">
                ...
                <el-select v-model="SysHosRegionVO.regionCode" placeholder="请选择">
                  <el-option v-for="item in this.regions" :key="item.id" :label="item.label" :value="item.id">
                  </el-option>
                </el-select>
              </div>
            </div>
            <div style="margin-top: 20px">
              ...
              <div class="right">
                <el-date-picker v-model="SysHosRegionVO.createdAt" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="选择日期">
                </el-date-picker>
            ...
            <!--<el-input-->
            <!--placeholder="请输入内容"-->
            <!--v-model="SysRegionCodeVO.regionCode"-->
            <!--v-bind:disabled="SysRegionCodeVO.regionCode!=''&&this.status=='modify'?true:false"-->
            <!--&gt;-->
            <!--</el-input>-->
            ...
            <div style="margin-top: 20px">
              ...
              <div class="right">
                <el-input placeholder="请输入内容" v-model="SysHosRegionVO.createdByName">
                </el-input>
...
<script>
  import {
    getHospitalDistrictNode,
    saveHospitalDistrictNode,
    disableHospitalDistrictNode,
    getNodeById,
    delHosRegionNode
  } from "@/api/phss/region/hos_district";
  import {
    getRegionNode
  ...
  export default {
    ...
    methods: {
      ...
      changeRegion() {
        // getRegionNode
        let params = {
            year: this.SysHosRegionVO.year,
            regionType: "03"
        }
        getRegionNode(params)
          .then(res => {
      ...
      save() {
        console.info("save", JSON.stringify(this.SysHosRegionVO));
        saveHospitalDistrictNode(this.SysHosRegionVO).then(res => {
          if (res.code == "1") {
      ...
      add(data) {
        ...
        this.SysHosRegionVO = {};
        console.info("当前节点", JSON.stringify(data));
        this.SysHosRegionVO.hosId = data.id;
        this.countyFlag = true;
      ...
      disable(data) {
        ...
        if(data.status || data.status == '1'){
          ...
          disableHospitalDistrictNode(data).then(res => {
            if (res.code == "1") {
      ...
      remove(node, data) {
        ...
        if(data.status || data.status == '1'){
          delHosRegionNode(data).then(res => {
            if (res.code == "1") {
