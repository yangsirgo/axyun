    ...
    <div class="outpcharge-main width100 height-button-main">
      <el-tabs class="height100" v-model="activeName">
        <el-tab-pane label="可申请" name="first">
          <div class="charge-box" v-loading="listLoad">
            <p class="no-data" v-if="tableData.length === 0">暂无数据</p>
            <div
              class="charge-item"
              v-for="(item,index) in tableData"
              :key="item.orderChargeDetailId"
            >
                ...
                <el-row>
                  <el-col :span="5">
                    <el-checkbox
                      ...
                      @change="orderChange($event,item.orderChargeDetailId,index)"
                    >
                      <span>{{item.orderName}}</span>
                    </el-checkbox>
                  </el-col>
                  <el-col :span="3">收费数量：{{ item.orderQuantity }}项</el-col>
                  <el-col :span="5" v-if="item.orderSource === '4' || item.orderSource === '2'">
                    申请数量：
                    <el-input-number
                      style="width:100px;"
                      v-model="item.refundOrderQuantity"
                  ...
                  <el-col :span="3">总金额：{{ formatNumber(item.totalMoney) }}元</el-col>
                </el-row>
              </div>
              <el-table
                :data="item.outpChargeDetailNewVOS"
                row-key="id"
                border
                style="width: 100%;"
              >
                <el-table-column width="45px">
                  <template slot-scope="scope">
                    <el-checkbox
                      ...
                      @change="orderItemChange($event,item.orderChargeDetailId,index)"
                    ></el-checkbox>
                  </template>
                </el-table-column>
