<template>
  <div class="height100">
    <div class="table-utils">
      ...
      <!--@click="printRecipe"-->
      <!--&gt;打印</el-button>-->
      <!--<span class="filter-item" style="margin-left: 22px">-->
      <!--<el-checkbox v-model="showAllRp" @change="showAllRpChange">全部医嘱项目</el-checkbox>-->
      <!--</span>-->
      <el-button
        type="primary"
        @click="submitCurRow()"
          ...
          >
            <template slot-scope="scope">
              ...
              <template v-else>
                ...
                <!-- 金额格式化 -->
                <template
                  v-else-if="item.prop == 'price' || item.prop == 'amt'"
                >
                ...
                <template v-else-if="item.prop === 'orderCat'">
                  <span>
                    <span
                      ...
                      :conditionMap="{
                        doctors_advice_code: scope.row.orderCat,
                      }"
                ...
                <template v-else-if="item.prop === 'execDeptNameChn'">
                  <span>{{
                    codeToText(scope.row.execDeptNameChn, drugStorageList)
                  }}</span>
                </template>
...
export default {
  ...
  methods: {
    ...
    westDrugsFunc(tableData = []) {
      tableData.forEach(item => {
        let qty =
          item.unitKey === item.packUnit
        ...
        item.templetDetailId = item.id;
        item.templetId = this.templetId;
        item.templetType = this.templetType;
        item.itemCat = item.orderCat;
        item.itemName = item.orderItemName;
        item.packageUnit = item.packUnit;
        item.oneDosage = item.onceDosage;
        item.executeDepartmentName = item.execDeptName;
      });
    },
    chmDrugFunc(tableData = []) {
      tableData.forEach(item => {
        console.log(item.unitKey, item.packUnit, item.qty);
        //数量换算
        // 换算最小单位数量
        let qty =
          item.unitKey === item.packUnit
    ...
    distributeData(tableData = []) {
      switch (this.templetType) {
        case "10": //西药
          this.westDrugsFunc(tableData);
          break;
        case "11": //中草药
          this.chmDrugFunc(tableData);
          break;
        default:
          this.nonDrugFunc(tableData);
      }
      return tableData;
    },
    async saveAllFunc() {
      const flag = this.tableRuleForm.tableData.every(
        item => item.hasOwnProperty("dataVersion") && item.orderItemName
      );
      if (!flag) {
        this.$message.warning("请填写明细！");
        return;
      }

      let ajaxData = this.tableRuleForm.tableData.filter(
        item => item.hasOwnProperty("dataVersion") && item.orderItemName
      );
      ...
      ajaxData = this.distributeData(ajaxData);
      try {
        let { code, msg, data } = await batchInsertDetails(ajaxData);
        if (code == 1) {
          this.$message.success("全部保存成功！");
          this.getList(false);
        } else {
          this.$message.error(msg || "全部保存失败");
        }
      } catch (error) {
        ...
        .then(async res => {
          if (res.code === 1) {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              // let dropRate2 = "";
              // if (
              //   item.dropRate && item.dropRate.indexOf("~") > -1
              // ) {
            ...
            if (this.templetType !== "10" && this.templetType !== "11") {
              ...
              for (let i = 0; i < tableData.length; i++) {
                ...
                innerItem.execDeptName = innerItem.executeDepartmentId;
              }
            }
            ...
            this.$emit("update:typeDisable", tableData.length > 0);
    ...
    addRow() {
      ...
      console.log(addRowEmpty, "addRowEmpty---");
    ...
    async select(row) {
      ...
      let data = {
        ...
        execDeptCode: row.execDeptCode,
        executeDepartmentId:
          Number(row.execDeptCode || JSON.parse(getRole()).deptId) || "",
    ...
    handleFreq(value, item) {
      ...
      this.$set(this.currentRow, "frequencyTimes", item.frequencyTimes);
      this.$set(this.currentRow, "frequencyUnit", item.cycleUnit);
      this.handleQty();
    },
    // 输入用药天数时，计算数量
    handleQty(value) {
