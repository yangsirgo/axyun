<template>
  <el-card class="height100 wrapperCls">
    <div class="topPart">
      <div class="tablePart">
        <div class="tablePartUtils">
          <div
            class="overflow-hidden search-box padding16"
          ...
          >
            ...
            <div class="overflow-hidden" style="font-size: 0">
              ...
              <el-button plain @click="splitFn" :disabled="splitFnStyle" :loading="splitFnLoading"
                >拆分</el-button
              >
              <el-button plain @click="splitAllFn" :disabled="splitFnStyle" :loading="splitAllFnLoading"
                >全部拆分</el-button
              >
            ...
            >
              <template slot-scope="scope">
                ...
                <template v-else-if="item.prop == 'allSum'">
                  <div
                    v-if="scope.row['allSum'] === 0"
                    class="totalDegreeGreen"
                  >
                    {{ scope.row["allSum"] }}
                  </div>
                  <div
                    v-else-if="scope.row['allSum'] > 0"
                    class="totalDegreeBlue"
                  >
                    {{ scope.row["allSum"] }}
                  </div>
                  <div v-else>--</div>
                ...
                <div v-else-if="item.prop == 'ouptDeptId'">
    ...
    <el-row class="bottomForm">
      <l-card-title class="title" style="padding: 16px">
      ...
      <div class="bottomForm-table">
        <el-table
          ...
          height="100%"
          style="width: 100%"
          @cell-click="radioChangeCell"
          @selection-change="recordChangeHandler"
        >
          <!-- <el-table-column type="selection" width="50"> </el-table-column> -->
    ...
    <el-dialog title="执行确认" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="" prop="execTime">
          <l-formt-title
            :required="true"
            label="选择执行时间"
            class="position-relative float-left"
        ...
        <el-form-item label="" prop="execId">
          <l-formt-title label="执行人" :required="true">
            <el-select
              v-model="form.execId"
              placeholder="请选择"
              @change="execPersonOption"
            >
              <el-option
                v-for="(item, index) in execPersonList"
                :key="index"
                :label="item.name"
                :value="item.value"
              ></el-option>
...
<script>
import { treatmentList } from "@/api/cis/treatment/injection";
...
import { getPrintData } from "@/api/cis/visit/visit";
...
export default {
  ...
  data() {
    return {
      splitFnLoading: false,
      splitAllFnLoading: false,
      splitFnStyle: false,
      findExecListIds: "",
      ...
      divisionOrdersParams: {
        ...
        execStatus: "",
        execTime: "",
        execType: "3", // 执行类型   0-输液  1-注射  2-皮试  3-治疗'
        ids: "",
        opentionType: "",
        remark: "",
        startDate: "",
      ...
      form: {
        ...
        startDate: "",
        endDate: "",
        execType: "3", //执行类型0-输液  1-注射  2-皮试  3-治疗',
        id: ""
      },
      diaFormRules: {
        execTime: [
  ...
  methods: {
    ...
    validatePrintData() {
      ...
      let arr = this.groupingPrint(data);
      return arr;
    },
    async printCard() {
        let orderId = this.validatePrintData();
        if (!orderId.length) {
          return;
        }
        for(let i in orderId) {
          await this.printCardFuc(orderId[i]);
        }
      },
    // zhengyawen 打印执行卡
    async printCardFuc(orderId) {
      try {
        let params = {
          orderStatus: 99,
          orderId,
        ...
        let num = Math.ceil(orderId.length / 6);
        let numList = [];
        for (let i = 0; i < num; i++) {
          params.pageNo++;
        ...
        // onPreviewData(numList, "门诊护士门诊治疗单");
      } catch (e) {
        console.log(e);
      }
    },
    ...
    recordChangeHandler(val) {
      /* let data = this.record.recordData;
      data.forEach(item => {
        this.$set(item, "tableRadio", 0);
      });

      if (val.length) {
        val.forEach(row => {
          this.radioChangeCell(row);
        });
      } */
    },
    // 点击一行 让复选框成为选中状态
    radioChangeCell(row) {
      // this.$refs.recordsTable.toggleRowSelection(row, true);
    ...
    splitFn() {
      this.$showLoading();
      this.splitFnLoading = true;
      if (this.selectedList.length < 1) {
        this.$hideLoading();
        this.splitFnLoading = false;
        this.$message.warning("请选择至少一条记录!");
        return false;
      }
      const orderStatusflag = this.selectedList.filter(
      ...
      console.log(this.idArr, "打印数据的");
      this.splitFnStyle = true;
      checkDivisionOrders({
        ids: this.idArr,
    ...
    splitAllFn() { 
      this.$showLoading();
      this.splitAllFnLoading = true;
      if (this.infusionTable.data.length == 0) {
        this.$hideLoading();
        this.splitAllFnLoading = false;
        this.$message.warning("列表中没有可以拆分的数据");
        return false;
      }
      let orderStatus4List = this.infusionTable.data.filter(
        item => item.orderStatus == 4
      );
      // console.log(orderStatusflag)
    ...
    handleSelectionChange(val) {
      ...
      // let orderStatus10List = val.filter(item=> item.orderStatus == 10)
      ...
      if (orderStatus10List.length > 0) {
        ...
        this.findExecListGroupNos = orderStatus10List
      ...
      console.log("this.findExecListIds", this.findExecListIds);
      let that = this;
      clearTimeout(iTime);
    ...
    doDivisionOrders() {
      ...
      nonDivisionOrders(this.divisionOrdersParams)
        .then(res => {
          if (res.code === 1) {
    ...
    getExecResult() {
      // console.log("role-res", this.$store.state);
      ...
      this.form.execTime = this.formatDate(dayDate);
    ...
    async confirmAfter(waitExecRp) {
      ...
      let flag = "";
      if (
        (this.form.execStatus == "3" || this.form.execStatus == "4") &&
        this.form.remark == ""
      ) {
        flag = "请填写备注";
      }
      try {
        let res = await nonExecConfirmation(this.form);
        if (res.code === 1) {
          flag = 1;
        } else {
          flag = res.msg || "执行失败";
    ...
    selectItem(list, row) {
      ...
      this.infusionTable.data.map(item => {
        if (row.groupNo === item.groupNo && item.id !== row.id && row.groupNo) {
