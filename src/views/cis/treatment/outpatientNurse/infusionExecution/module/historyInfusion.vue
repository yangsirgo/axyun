    ...
    <div class="right height100 position-relative padding16">
      <div class="height100 position-relative" v-loading="isLoading">
        <div class="overflow-hidden search-box" style="padding-bottom: 0px;">
          <div class="overflow-hidden" style="height: 45px;font-size:0;">
            <l-formt-title label="输液开始时间" style="width: 35%;margin-right: 8px" class="position-relative float-left">
              <el-date-picker
                v-model="dateRange"
            ...
            <el-button type="primary" plain @click="reset" style="vertical-align: 10px">重 置</el-button>
          </div>
        </div>
        <div class="right" style="height:calc(100% - 35px)">
          <div style="margin-top: 5px;height:calc(100% - 12px)">
            <el-table
              height="100%"
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        findHisStoryList(params).then(res => {
          if (res.code === 1) {
            ...
            this.stayExecute.data = this.visitCodeSort(res.data);
          } else {
            this.$message.error(res.msg);
          }
          // this.loading = false;
      ...
      visitCodeSort (list1){
        ...
        let filterList = [];
        let tableList = [];
        visitCodeList = list1.map(item => {
          // 获取所有组
          return item.visitCode;
        });

        visitCodeList = Array.from(new Set(visitCodeList));
        visitCodeList = visitCodeList.filter((item, index) => {
          // 数组去undefind
          return item !== undefined && item != "";
        });

        console.log("visitCodeList:::", visitCodeList);

        visitCodeList.forEach((item, index) => {
          //遍历 成map结构 分组
          // list = [];
          filterList = list1.filter((sonItem, sonIde) => {
            return item === sonItem.visitCode;
          });
          list.push({
            key: item,
            value: filterList
        ...
        list.map(item => {
          ...
          let groupingList = this.grouping(itemList);
          // console.log("groupingList:::", groupingList);
          tableList = tableList.concat(groupingList);
        });
