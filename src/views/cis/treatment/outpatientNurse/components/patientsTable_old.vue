  ...
  <div class="height100 table_tabs_wrapper outpatientNurse">
    ...
    <el-collapse v-model="activeNameCollapse" accordion>
      <el-collapse-item
        :title="
          radio == 'first'
            ? '待执行（' + showData.wait.length + '）'
            : '已完成（' + showData.received.length + '）'
        "
        name="1"
      >
        <l-table
      ...
      <!-- <el-collapse-item title="已完成" name="2" v-if="radio=='2'">

      </el-collapse-item> -->
      <el-collapse-item title="皮试中" name="3">
        <div class="wrapperCls">
          ...
          <div class="infinite-list-wrapper" v-loading="loadingSkin">
            <div
            ...
            >
              ...
              <p v-if="!loadingSkin && noMore" class="load-list-bottom">
                {{ $t("base.noMore") }}
              </p>
            </div>
          </div>
...
export default {
  ...
  components: { skinTesting, countDown },
  methods: {
    // 皮试中
    callback() {
      console.log("到点儿了");
        ...
        .then(res => {
          ...
          if (res.code == 1) {
            ...
            if (this.pageParams.pageNo === 1) {
              ...
              // this.changeRecPatientData(this.purchaseList[0] || {})
            } else {
    ...
    submitForm(formName) {
      this.radio = this.ruleForm.region;
    ...
    loadData(noLoading) {
      if (this.radio == "second") {
        // this.$refs.skintesting.loadTable()
        // return
      }
      if (!noLoading) {
        //控制loading显示 与 不显示
      ...
      try {
        newFindPatientListExecutoryContractByExecType(params, false).then(
          res => {
            ...
            this.showData.wait = res.data;
            this.showData.received = res.data;
            console.log("this.showData.wait", this.showData.wait);
            ...
            if (!targetRow) {
              ...
              if (
                this.$router.currentRoute.path.indexOf("/cisOne/cisThree") ===
                -1
              ) {
