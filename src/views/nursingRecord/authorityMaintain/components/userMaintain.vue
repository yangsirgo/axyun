  ...
  methods: {
    ...
    async getRoleData() {
      ...
      try {
        ...
        let res = await queryUsers(params);
        if (res.code == "1") {
        ...
        this.tlm_isLoading0 = false;
        this.$message.error(err.msg || "获取用户数据失败");
      }
    },
    load0() {
      const _this = this;
    ...
    async getDocRole() {
      ...
      try {
        let params = { userId: this.currentRole.id };
        let res = await getDocUserRightByUserId(params);
        if (res.code == "1") {
          let data = [...res.data];
          for (let i in data) {
          ...
          this.$message.error(res.msg || "获取用户文书权限失败");
        }
        this.tlm_isLoading = false;
      } catch (error) {
        this.tlm_isLoading = false;
        this.$message.error(error.msg || "获取用户文书权限失败");
      }
    },
    //已选用户权限与右侧文书列表做匹配并切换选中状态
    comparison() {
      for (let i in this.docRoleData) {
        for (let j in this.docInfoData) {
          if (
            this.docInfoData[j]["docCode"] == this.docRoleData[i]["docCode"]
          ) {
            this.$nextTick(() => {
    ...
    comparison1() {
      let docInfoDataUnit = this.docInfoData.slice(
      ...
      for (let i in this.docRoleData) {
        for (let j in docInfoDataUnit) {
          if (docInfoDataUnit[j]["docCode"] == this.docRoleData[i]["docCode"]) {
            this.$nextTick(() => {
              this.$refs.docTable.toggleRowSelection(
                this.docInfoData[
                  Number((this.pageNo - 1) * this.pageSize) + Number(j)
                ],
                true
              );
            });
            ...
            for (let v in this.newSelection) {
              if (
                docInfoDataUnit[j]["docCode"] == this.newSelection[v]["docCode"]
              ) {
      ...
      setTimeout(async () => {
        if (!_this.totalPage) {
          ...
          for (let i in _this.docInfoDataUnit) {
            _this.$set(
              _this.docInfoData,
              _this.docInfoData.length,
              _this.docInfoDataUnit[i]
            );
          }
          //比对渲染
          _this.comparison1();
    ...
    change(value) {
      ...
      if (
        docInfoRow["docBrowse"] == false &&
        docInfoRow["docAdd"] == false &&
        docInfoRow["docModify"] == false &&
        docInfoRow["docDelete"] == false
      ) {
    ...
    async save() {
      ...
      this.$showLoading();
      try {
        let data = await saveDocUserRight(params);
        if (data.code == "1") {
          this.$message("更新成功");
          this.getDocRole();
        } else {
