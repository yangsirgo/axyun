  ...
  <div class="width100 height100 filingAndChangeindex">
        <div class="bg-contain recordCreate width100 height100">
          ...
          <div class="width100 height100">
            <div class="card-cont1 width100 m-contain bottom-con third-con">
...
export default {
  ...
  watch: {
    ...
    patientItem: {
      handler(n) {
        if(n && n.length) {
          // let {  } = deepClone(n[0]);
          this.patientListFunData(deepClone(n[0]), true);
        }
      },
      immediate: true,
      deep: true
  ...
  methods: {
    ...
    addId(a) {
      ...
      //zhengyawen 校验是否跟原先的证件一样，一致的话把cardId等字段赋值到cardForm上
      let archCardList = this.patientItem[0].archCardList;
      if (archCardList.length) {
        for (let i in archCardList) {
          if (archCardList[i].cardData == this.cardForm.cardData && archCardList[i].cardType == this.cardForm.cardType) {
            this.cardForm = {
              ...archCardList[i]
            }
          }
