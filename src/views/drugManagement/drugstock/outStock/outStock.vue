<template>
  <el-card class="height100 position-relative width100" style="padding: 16px">
      ...
      <div class="css_outStockinfo">
            ...
            <l-formt-title :disabled="
              ...
              <el-input :disabled="
                  !isNewAddStatus
                    ...
                    : false
                "
                v-model="outStockinfo_main.recordRemark" maxlength="200" show-word-limit placeholder></el-input>
            </l-formt-title>
          </el-col>
        </el-row>
        <el-row :gutter="6" class="infotext main-info" style="margin-bottom: 8px">
          ...
          <el-col :span="6">
            <span>
              出库药库：
              <span columns="STORAGE_NAME" :conditionMap="{ STORAGE_CODE: outStockinfo_main.storageCode }" tableName="hrp_drgstrg"
        ...
        <el-row class="table-btn">
          ...
          <el-button type="text" :disabled="isShadow" class="float-left cursor-pointer margin-right-10" v-if="outStockinfo_main.outType === '1'"
            @click="newlist">
        ...
        <!-- 出库单table -->
        <el-row class="tablerow position-absolute" :style="outStockinfo_main.status !== '1' ? 'top:175px' : 'top:140px'">
          <l-tab-change :tableData="tableData" class="height100">
            <el-table ref="singleTable" height="100%" :data="tableData" border stripe style="width: 100%" v-loading="mainLoading"
              highlight-current-row @current-change="handleCurrentChange" @selection-change="handleSelectionChange"
              @cell-dblclick="dblclick" @header-dragend="repair">
              <el-table-column v-if="
                  outStockinfo_main.status !== '1' &&
                  outStockinfo_main.status !== '2'
                "
                type="selection" width="40" key="t1"></el-table-column>
              <el-table-column key="t2" width="200" show-overflow-tooltip label="药品名称" fixed>
              ...
              <el-table-column key="t3" prop="spec" label="规格" show-overflow-tooltip width="200"></el-table-column>
              <el-table-column key="t4" label="进价(元)" width="120" align="right" prop="purchasePrice"
                show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{ formatNum(scope.row.purchasePrice) }}</span>
                </template>
              </el-table-column>
              <el-table-column key="t5" label="售价(元)" width="120" align="right" prop="price" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{ formatNum(scope.row.price) }}</span>
                </template>
              </el-table-column>
              <el-table-column key="t7" width="120" :label="outStockinfo_main.outType === '2' ? '库损数' : '库发数'"
                show-overflow-tooltip>
                <template slot-scope="scope">
                  <div class="noWrap">
                    ...
                    <template v-else>
                      <el-input v-model="scope.row.kfOut" @keyup.native="outFitChange(scope.$index)" placeholder></el-input>
                    </template>
              ...
              <el-table-column key="t10" label="生产厂商" width="200" show-overflow-tooltip>
                <template slot-scope="scope">
              ...
              <el-table-column key="t12" label="生产批号" prop="batchNum" width="140" show-overflow-tooltip></el-table-column>
              <el-table-column key="t13" label="进价总额(元)" width="120" align="right" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{
                    formatNum(
                      Math.round(
              ...
              <el-table-column key="t14" label="售价总额(元)" width="120" align="right" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{
                    formatNum(
                      Math.round(
              ...
              <el-table-column key="t16" label="有效期" width="200" prop="expDate" show-overflow-tooltip></el-table-column>
          ...
          <div v-if="isShadow" @click="validateTips" style="position: absolute" class="width100 height100 shadow"></div>
        </el-row>
      </div>
    </div>
    ...
    <div class="bottom-btn height-button-bottom">
      ...
      <el-button v-if="outStockinfo_main.status == '0'" class="less-btn float-right" type="primary" @click="save(true)"
    ...
    <!-- 修改批次 弹窗 -->
    <l-dialog :visible.sync="drugVisible" :title="this.drugTitleName" width="1000px" @confirm="conserve" @cancel="drugClose">
      <template #content>
        <el-form label-position="top" inline class="padding20">
          <el-table ref="diagTable" :data="searchTable" border stripe width="100%" v-loading="tableLoading"
            element-loading-text="列表正在加载中" height="300">
            <el-table-column prop="drugName" label="药品名称" show-overflow-tooltip></el-table-column>
            <el-table-column label="进价(元)" prop="purchasePrice" width="120" align="righr">
              <template slot-scope="scope">
                <span>{{ formatNum(scope.row.purchasePrice) }}</span>
              </template>
            </el-table-column>
            <el-table-column label="售价(元)" prop="price" width="120" align="righr">
              <template slot-scope="scope">
                <span>{{ formatNum(scope.row.purchasePrice) }}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="库发数" prop="backQuantity" width="120">
              <template slot-scope="scope">
                <div class="noWrap">
                  <el-input class="number-input" v-model="scope.row.kfOut" placeholder @keyup.native="textValueDia($event, scope.$index)"></el-input>
                  <span :val="scope.row.drugUnit" code="DrugUnit" v-codeTransform></span>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="生产批号" prop="batchNum" width="120" show-overflow-tooltip></el-table-column>
    ...
    <!-- 申请列表弹窗 -->
    <outlist :id="outStockinfo_main.id" v-if="modelVisible" :modelVisible.sync="modelVisible" :status="outStockinfo_main.status"
      :applyId="outStockinfo_main.applyId" @emitModel="emitModel" @send-data="getListData" :isNewAdd="isNewAddStatus"></outlist>

    <!-- 入库单列表 -->
    <l-dialog title="入库单列表" :visible.sync="dialogFormVisible" width="1000px" @confirm="determine" @cancel="dialogFormVisible = false">
      <template #content>
        <div class="pur-box">
          <div class="pur-list">
            <div v-for="(item, index) in purlist" :class="['pur-item', curPurItem.id === item.id ? 'active' : '']" :key="index"
          ...
          <div class="pur-table">
            <el-table :data="purTableData" v-loading="purTableLoad" height="100%" border stripe style="width: 100%">
              <el-table-column prop="drugName" header-align="center" label="药品名称"></el-table-column>
              <el-table-column prop="spec" header-align="center" label="规格"></el-table-column>
              <el-table-column prop="purchasePrice" label="进价" header-align="center" align="right">
                <template slot-scope="scope">
                  <span>{{ formatNum(scope.row.purchasePrice) }}</span>
                </template>
              </el-table-column>
...
<script>
  ...
  export default {
    ...
    computed: {
      isShadow() {
        return (
          !this.outStockinfo_main.outType && !this.outStockinfo_main.deptCode
        );
      },
      noMore() {
        return (
    ...
    methods: {
          ...
          confirm: async (action, instance, done) => {
            ...
            try {
              ...
              if (res.code === 1) {
                ...
                this.isNewAddStatus && this.refreshList();
                await this.submitData(params, scuMsg);
              } else {
      ...
      async conserve() {
        ...
        this.searchTable.forEach((item) => {
          if (item.kfOut !== "") {
            let data = {
              ...item,
              outQuantity: outfitToBluk(item.kfOut, 0, item.packQuantity),
              outInventoryNum: this.outStockinfo_main.outInventoryNum,
          ...
          for (let i = len; i >= 0; i--) {
            for (let j = 0; j < this.selectedSearchTableList.length; j++) {
              if (
                ...
                console.log(this.tableData[i].inId, i);
      ...
      // 库发数输入时根据整装数量转换成散装数量
      outFitChange(index) {
        this.validateResult = new RegExp("^[0-9][0-9]*$").test(
      ...
      determine() {
        let list = this.purTableData.map((item) => {
          let {
            out,
            bl
          } = blukToOutfit(item.inQuantity, item.packQuantity);
          return {
            ...item,
