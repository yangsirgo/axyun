  ...
  <div class="wrapper">
    ...
    <div class="float-left main">
      <div class="main_wrap" :style="{'margin-right': toolBoxExpanded ? '298px' : '56px'}">
        <flow-menu :patientCardType="patientCardType">
          <slot name="content">
            <l-main ref="main" @search="search" :purchaseInfo="purchaseInfo" :recordStatusList="recordStatusList"
              :supplierList="supplierList" :drugStorageList="drugStorageList"></l-main>
          </slot>
        </flow-menu>
      </div>
    </div>
    ...
    <div class="float-left left">
      <left ref="left" @selectIdChange="selectIdChange" @newAdd="newAdd" :recordStatusList="recordStatusList"
        :supplierList="supplierList" :drugStorageList="drugStorageList"></left>
    </div>

    <!--    右侧页面-->
    <div class="float-left right" :style="{'width': toolBoxExpanded ? '287px' : '46px', 'margin-left': toolBoxExpanded ? '-287px' : '-46px'}">
      <tool-box pageName="drugPurchase" :boxName="defaultBoxName" :modules="boxs" @message="toolBoxMessage" :open.sync="toolBoxExpanded" />
    </div>
  </div>
</template>
