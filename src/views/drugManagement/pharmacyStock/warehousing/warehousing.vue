              ...
              <template slot-scope="{ row }">
                <span>
                  {{
                    formatNum(
                      Math.round(
                        (row.purchasePrice * 100 * row.inQuantity) /
                          row.packQuantity
                      ) / 100
                    )
                  }}
              ...
              <template slot-scope="{ row }">
                <span>
                  {{
                    formatNum(
                      Math.round(
                        (row.price * 100 * row.inQuantity) / row.packQuantity
                      ) / 100
                    )
                  }}
                </span>
          ...
          >
            ...
            >
              ...
              <el-row>
                ...
                <el-col :span="12">
                  <p :title="formatNum(item.totalAmount)">
                    总进价：{{ formatNum(item.totalAmount) }}
                  </p>
                </el-col>
              </el-row>
              <!-- <el-row>
...
import LocalDrugSelector from "@/components/LocalDrugSelector/index.vue";
  ...
  methods: {
      ...
      this.$confirmFunc({
        ...
        confirm: async (action, instance, done) => {
          ...
          let dataList = this.tableData.map(item => {
            if (item.unitSaleFlg == 0 && (item.bluk === "" || !item.bluk)) {
              item.bluk = 0;
            } else if (item.unitSaleFlg == 1 && item.outfit === "") {
              item.outfit = 0;
            }
            return {
              ...item,
    ...
    adddrug() {
      ...
      this.$nextTick(function() {
        ...
        if (this.tableData.length !== 0 && this.inputDomsLength === 0) {
          this.inputDomsLength = this.initInputDOM().length - 5;
        }
        this.inputDoms[this.inputDoms.length - 5].focus();
      });
    },
    enterlast() {
      // 键盘Tab 到最后一个 触发回调事件
    ...
    determine() {
      let list = this.purTableData.map(item => {
        let { out, bl } = blukToOutfit(item.saveQuantity, item.packQuantity);
        return {
          ...item,
          inQuantity: item.saveQuantity,
          outfit: out,
          bluk: bl,
          expDate: "",
          batchNum: "",
    ...
    batchSelect(row) {
      this.$set(this.tableData, this.tableindex, {
        ...
        expDate: this.dayjs(row.expDate).format("YYYY-MM-DD"),
