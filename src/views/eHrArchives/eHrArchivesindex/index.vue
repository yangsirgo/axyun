...
export default {
  ...
  data() {
    return {
      // readTheDataList
      // 建档与变更变化
      readTheDataStyle: true,
      readTheDataListObj: this.readTheDataList,
      // 控制标签的显示和隐藏
      displayStyle: false,
  ...
  watch: {
    readTheDataListObj: {
      handler(newVal) {
        console.log(
          "====readTheDataListObj=======gggggggggggggttt",
          this.readTheDataListObj
        );
        alert(123);
      },
      deep: true
  ...
  created() {
    if (this.readTheDataList) {
      this.patientArchiveInfoVO = this.readTheDataListObj;
      this.readTheDataStyle = false;
    }
  },
  methods: {
    // 左侧列表传过来的患者的基本信息数据
    patientListFunData(item, showData) {
      console.log(item);
      this.patientArchiveInfoVO = item.patientArchiveInfoVO;
      this.drawer = false;
    },
