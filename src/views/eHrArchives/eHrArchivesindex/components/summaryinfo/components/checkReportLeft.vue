...
<script>
import { selectEasApplyExam } from '@/api/eHrArchives/eHrArchives';
export default {
  name: 'index',
  props: ['visitCode'],
  data() {
  ...
  methods: {
    ...
    // 左侧列表
    async selectEasApplyExam() {
      let result = await selectEasApplyExam(this.objData);
      if (result.code == '1') {
