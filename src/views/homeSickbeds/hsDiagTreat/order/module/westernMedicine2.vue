          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span v-if="typeof scope.row.status === 'undefined'">新增</span>
  ...
  methods: {
    ...
    handleQty() {
      this.$nextTick(() => {
        ...
        let orderItem =
          this.drugItem && this.drugItem.itemId ? this.drugItem : currentRow;

        let qty = this.handleQtyUtil(
          useDay,
          freq,
          onceDosage,
          orderItem.dosageMeasure,
          orderItem
        );
    ...
    handleQtyUtil(
      ...
      if (
        ...
        qty = (useDay * times * onceQty) / currentItem.pkgMeasure;
      }
    ...
    async selectWrapper(row) {
      ...
      // 药品 且库存为0 禁止用户选中，并提示用户
      if (
        row.id &&
        (row.drugStock == 0 || typeof row.drugStock === "undefined")
      ) {
    ...
    checkDupFunc(val) {
      console.log("tableData:::", this.tableRuleForm.tableData);
      console.log("val:::", val);
      // 判断是否添加相同医嘱
    ...
    //清空药品搜索框
    clearLInputTable() {
      // let refs = this.$refs["lInputTable"] ? this.$refs["lInputTable"] : [];
      // let List = Array.isArray(refs) ? refs : [refs];
      let currentRow = this.tableRuleForm.tableData[this.currentRowIndex];
      // console.log("list", List);
      // List.forEach(item => {
      //   console.log(item.$el.getAttribute("data-id"));
      //   if (item.$el.getAttribute("data-id") == currentRow.row_id) {
      //     item.clear();
      //   }
      // });
    ...
    addRow() {
      ...
      this.$nextTick(() => {
        ...
        this.setComponentFocusFunc("lInputTable", obj.row_id);
    ...
    setComponentFocusFunc(refName, targetId) {
      ...
      // refs 方式 如果有fixed列会失效，直接用操作DOM处理
      this.$nextTick(() => {
        console.log(this.$refs.mainTableRuleForm);
        setTimeout(() => {
    ...
    unitChange(value, row) {
      ...
      row.unitFlag = row.unit === row.pkgUnit ? "1" : "0";
    ...
    isZanCunStatus(list = []) {
      return list.every(item => {
        return item.status == "0"; //只有暂存状态能提交
      });
    },
    //成组校验
    bunchingValidate() {
