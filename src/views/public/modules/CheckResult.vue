            ...
            <template slot="title">
              <div class="width100">
                <div
                  ...
                  type="primary"
                >{{ $t("base.quote") }}</div>
                <div class="sbj">{{ item.itemName }}</div>
              </div>
            </template>
            <template slot="content">
              <div>
                <div class="data-key">检查表现：</div>
                <div class="data-value">{{ item.examDescription }}</div>
                <div class="data-key">检查结论：</div>
                <div class="data-value">{{ item.examResult }}</div>
              </div>
            </template>
          </l-collapse-item>
        </el-collapse>
...
import { getExaList } from "@/api/tools/CheckResult.js";
...
export default {
  ...
  methods: {
    ...
    // 引用
    quote(data) {
      this.$emit("message", "insertCheck", data);
    },
    // 加载详情
    loadDetail(data) {
