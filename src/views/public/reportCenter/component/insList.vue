  ...
  <div class="container">
    ...
    <div class="content-loading" v-loading="loading" v-show="receivePatientData.patientId">
      ...
      >
        <el-submenu v-for="(item,index) in categoryList" :index="''+index" :key="index">
          <template slot="title">
            ...
            <span class="type-menu-num">{{item.countExam}}次</span>
          </template>
          <el-menu-item
            v-for="(item2, idx) in item.examPublishList"
            :key="item2.examPublishId"
            :index="''+index+ '-' +idx"
            @click="chooseReport(item2)"
          >
            <template>{{item2.examDate}}</template>
          </el-menu-item>
        </el-submenu>
      </el-menu>
      <el-timeline class="report-timeline" v-show="typeVal === '1'">
        ...
        >
          <el-card
            v-for="card in item.examPublishList"
          ...
          >
            <h4>{{card.examItemName}}</h4>
            <p>{{card.barCode||'--'}},{{card.examDeptName||'--'}},已发布</p>
          </el-card>
        </el-timeline-item>
        <!-- <el-timeline-item timestamp="历史报告" placement="top">
          <router-link class="link-style" :to="$route.path">更多报告</router-link>
        </el-timeline-item>-->
      </el-timeline>
    </div>
  </div>
...
import { getExamData2 } from "@/api/reportCenter/report";
...
export default {
  ...
  watch: {
    ...
    categoryList: {
      ...
      handler(val) {
        if (val.length > 0) {
          this.chooseReport(this.categoryList[0].examPublishList[0]);
        }
      }
    }
  },
  methods: {
    async loadData2() {
      ...
      try {
        let { code, data, msg } = await getExamData2(params);
        if (code === 1) {
    ...
    // 获取选择列表子项
    chooseReport(data) {
      this.showItemId = data.examPublishId;
