    ...
    <div id="results">
      <ul class="macro-list">
        <li
          ...
          :key="macro.mrShareEleId"
          @dblclick="addMacro(macro)"
        >
          {{ macro.mrShareEleNm }}
        </li>
      </ul>
...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      document.getElementById("results").style.height =
        document.getElementById("macrosLibrary").offsetHeight -
  ...
  methods: {
    ...
    async addMacro(macro) {
      ...
      this.editor.execCommand("insertmacros", {
        id: macro.mrShareEleId,
        code: macro.mrShareEleCd,
        name: macro.mrShareEleNm,
        isSecret: macro.securityF == 1
      });
    }
  }
};
