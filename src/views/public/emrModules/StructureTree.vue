    ...
    <div id="treeBox" class="tree-box">
      ...
      >
        <span class="custom-tree-node" slot-scope="{ node, data }">
          <span>{{ data.name }}</span>
          <span class="ellipse" v-if="editor.options.editorModel == 'design'">
            ...
            <div class="opts">
              <el-button
                type="text"
                size="mini"
                v-if="node.data.axplugins !== 'prescription'"
  ...
  methods: {
    calcLayout() {
      if (null != document.getElementById("treeBox")) {
        document.getElementById("treeBox").style.height =
          document.getElementById("structureTree").offsetHeight -
    ...
    getTreeData(isRefresh) {
      if (
        !document.getElementById("treeBox") ||
        parseInt(document.getElementById("treeBox").style.height) == "0"
      ) {
        this.calcLayout();
      }

      var treeData = listToTree(
        this.editor.execCommand("structuretree") || [],
        "id",
        "pId"
      ).list;
