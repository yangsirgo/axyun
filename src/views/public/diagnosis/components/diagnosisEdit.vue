            ...
            <el-row style="margin-top: 20px">
              <LFormtTitle label="ICD诊断描述" labelWidth="99" style="width:100%">
                <el-form-item label="" label-width="" prop="diagIcd">
                  <el-input v-model="accountRechargeForm.diagIcd" placeholder=""
...
<script>
  ...
  export default {
    ...
    watch: {
      asd() {
        this.accountRechargeForm = this.asd;
      }
    },
    created() {
      this.accountRechargeForm = this.asd;
      this.accountRechargeForm.startTime = new Date().format("yyyy-MM-dd HH:mm");
    },
    methods: {
      confirm() {
        this.$refs["formElem"].validate(async (valid) => {
          if (valid) {
            let res = await updateDiag(this.accountRechargeForm);
