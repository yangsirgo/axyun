            ...
            >
              <el-form-item>
                <l-formt-title label="临床诊断" :isMultiRow="true">
                  <el-input
                    v-model="diagformdata.diagName"
                    maxlength="150"
                  ></el-input>
                </l-formt-title>
              </el-form-item>
              <el-form-item>
                <l-formt-title label="简要病史" :isMultiRow="true">
                  <el-input
                    type="textarea"
                    resize="none"
            ...
            <div class="scrollheight">
              ...
              >
                ...
                <el-table-column
                  ...
                  min-width="80"
                ></el-table-column>
                <!-- <el-table-column prop="bodyPartsName" label="部位" min-width="80"> -->
                <el-table-column prop="position" label="部位" min-width="80">
                  <template slot-scope="scope">
                    <span
                      v-codeTransform
                      code="BodyPosition"
            ...
            <div class="scrollheight">
              ...
              >
                ...
                <el-table-column
                  ...
                  min-width="120"
                ></el-table-column>
                <!-- <el-table-column prop="bodyPartsName" label="部位11" min-width="80"> -->
                <el-table-column prop="position" label="部位" min-width="80">
                  <template slot-scope="scope">
                    <!-- <span>{{scope.row.position || ""}}</span> -->
                    <!-- <span>{{scope.row.bodyPartsName || "--"}}</span> -->
                    <span
                      v-codeTransform
                      code="BodyPosition"
                ...
                <el-table-column prop="isUrgent" label="加急" min-width="90">
                  <template slot-scope="scope">
                ...
                <!-- <el-table-column prop="bodyPartsName" label="部位" min-width="120">
                    <template slot-scope="scope">
                      <span>{{scope.row.bodyPartsName}}</span>
                    </template>
                </el-table-column>-->
...
export default {
  data() {
    return {
      ...
      diagformrules: {
        ...
        historyProfile: [
          { required: true, message: "请输入病史概要", trigger: "blur" }
        ],
        physicalExamination: [
    ...
    "selectedList.length": {
      ...
      handler(val) {
        ...
        if (!val) {
          ...
          this.diagformdata.advicePrompt =
            this.selectedList[0].adviceItemJson.advicePrompt || "";
        }
      }
    },
    filterText(val) {
  ...
  methods: {
   ...
   addHistoryProfile (jwss, xbss){
    ...
    // 替换 historyProfile 简要病史中的 换行和分号
    replaceHistoryProfile (historyProfile){
      return historyProfile.replace(/;\n/g,'');
    },
    deptChange(val, obj) {
    ...
    //     if(tree.adviceItemJson){
    //       tree.adviceItem = JSON.parse(tree.adviceItemJson)
    //     } else {

    //     }
    //   }else{
    //     for(var i=0;i<tree.children.length;i++){
    //       this.handleTreeData(tree.children[i])
    //     }
    //   }
    // },
    ...
    async applicationSub() {
      ...
      this.selectedList.forEach(item => {
        ...
        item.historyProfile = this.replaceHistoryProfile(this.diagformdata.historyProfile);
    ...
    cancelSelect() {
      ...
      console.log("this.applyListt======================", this.applyList);
    ...
    async getHistoryApplyList() {
      ...
      try {
        ...
        params["pageSize"] = "300";
        params["pageNo"] = "1";
        let result = await historyListExam(params);
    ...
    async getListByParamNotDelete() {
      try {
        let params = {};
        console.log(
          "getListByParamNotDelete-receivePatientData",
        ...
        let result = await getListByParamNotDelete(params);
        console.log("getListByParamNotDelete-result", result);
        if (result.code == "1") {
          // diagName
          this.diagformdata.diagName = result.data
            .map(item => item.diagName)
            .join("；");
          // this.diagformdata.diagName
          console.log("this.diagformdata.diagName", this.diagformdata.diagName);
        }

        let res = await getInfoByEnId(this.receivePatientData.visitCode, "01");
        console.log("getInfoByEnId-res", res);
        if (res.code == "1") {
          this.diagformdata.historyProfile = this.addHistoryProfile(res.data.paragraph.jws,  res.data.paragraph.xbs);
            // res.data.paragraph.jws + ";" + "\n" + res.data.paragraph.xbs;
          this.diagformdata.physicalExamination = res.data.paragraph.tgjc;
        }
      } catch (error) {}
    },
    //存为模板弹窗
    ...
    noteInput(val) {
      this.$forceUpdate();
      this.$set(this.currentLeftRow, "advicePrompt", val);
    },
    //作废申请
    backout() {
      let _self = this;
      ...
      this.$confirm("", "", {
        ...
        .then(async () => {
          let ids = _self.selectHistoryList.map(item => item.applyId).join(",");
          console.log(
          ...
          let result = await backoutApplyExam(params);
          if (result.code == 1) {
            this.$message({
              type: "success",
              message: "作废成功!"
    ...
    conserve() {
      ...
      for (let i = 0; i < this.applyList.length; i++) {
        let item = {
          ...
          itemCat: "5", //this.applyList[i].itemCat, // 检验8，检查5
          ...
          bodyPartsName: this.applyList[i].position, // 部位
          checkMethod: this.applyList[i].checkMethod, // 检查方法
    ...
    async printClickOld() {
      ...
      let res = await applyPrint({
        visitCode: this.receivePatientData.visitCode,
        applyId: this.printData[0].applyId,
        patientId: this.receivePatientData.patientId,
        type: "examApply"
      });
      if (res.code === 1) {
        data = res.data;
        onPreview(data, "检查申请单罗湖");
      }
    },
    async printClick() {
      if (this.printData.length < 1) {
        this.$message.error("请选择记录再进行打印");
        return false;
      }
