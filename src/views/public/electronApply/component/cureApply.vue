    ...
    rowClick(row) {
      ...
      if (row != null) {
        if (row != null) {
          //显示第一项
          if (row.state == 3) {
            this.rightdata.active = 2;
            this.rightdata.activeStepList[0].data.time = row.begaintime;
            this.rightdata.activeStepList[0].data.name = row.appDoctor;
            this.rightdata.activeStepList[1].data.time = row.execTime;
            this.rightdata.activeStepList[1].data.name = row.execDoctorName;
          } else {
            this.rightdata.active = 1;
            this.rightdata.activeStepList[0].data.time = row.begaintime;
            this.rightdata.activeStepList[0].data.name = row.appDoctor;
            this.rightdata.activeStepList[1].data.time = "";
            this.rightdata.activeStepList[1].data.name = "";
          }
        } else {
          this.clearStepList();
        }
    ...
    async sent() {
      ...
      this.checkedList.forEach(function(item) {
        if (typeof applyParamData["m" + item.id] == "undefined") {
          ...
          applyParamData["m" + item.id]["applyType"] = _self.templateType;
    ...
    conserve() {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          templateData["deptOrPerson"] = this.saveType;
          let paramData = {};
          let submitData = [];
          this.checkedList.forEach(function(item) {
        ...
        if (result.code == "1") {
          result.data.forEach(function(item) {
            ...
            newData.push({
              ...
              id: item.examApplyId, //id
              execTime: item.execTime, //执行时间
              execDoctorName: item.execDoctorName //执行医生
            });
          });
