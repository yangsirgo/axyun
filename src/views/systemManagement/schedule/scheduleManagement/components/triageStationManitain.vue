      ...
      <div class="form">
        <el-form :model="form" :rules="rules" ref="form" label-width="0">
          ...
          <el-form-item prop="locationMac">
            <l-formt-title label="MAC地址" required>
              <el-input
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <!-- 可自定义内容 -->
              <span v-if="item.prop === 'recordStatus'">{{
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        locationMac: [{ required: true, message: "请输入", trigger: "blur" }],
  ...
  methods: {
    ...
    delItem(data) {
      this.$confirmFunc({
        titleText: "提示",
        contentText: "此操作将永久删除该分诊站!",
        confirm: (action, instance, done) => {
          this.$showLoading();
          scheManagement
            .delStation(data.stationId)
            .then(res => {
              this.$hideLoading();
              done();
              if (res.code === 1) {
