<template>
  <el-card class="personnelFixedPoint height100">
    <div class="form-area">
      <el-row :gutter="10">
        <el-col :span="5">
          <SZReadCard
        ...
        <el-col :span="3">
          <LFormtTitle label="定点医院名称">
            <el-input v-model="form.fmName"></el-input>
          </LFormtTitle>
        </el-col>
        <el-col :span="4">
    ...
    <div class="table-cont">
      ...
      >
        <el-table :data="tableData" border width="100%" height="100%">
          <el-table-column
            v-for="(item, index) in personnelFixedPointColumns"
            :key="index"
            :prop="item.prop"
            :label="item.label"
...
import { personnelFixedPointColumns } from "../common/personnelFixedPoint.js";
import service from "@/api/medicalInsuranceNew/personnelFixedPoint.js";

export default {
  name: "personnelFixedPoint",
  components: {
    SZReadCard,
    addForm,
  },
  ...
  computed: {
    personnelFixedPointColumns() {
      return personnelFixedPointColumns;
    },
  },
  created() {
    this.search();
  ...
  methods: {
    ...
    async loadTable() {
      ...
      try {
        let res = await service.specHosFind(params);
        if (res.code == 1) {
    ...
    async cancelRegSure() {
      ...
      try {
        ...
        let res = await service.cancalspecRecord(params);
        if (res.code === 1) {
