<template>
  <el-card class="slowDisease height100">
    <div class="form-area">
      <el-row :gutter="10">
        <el-col :span="5">
          <SZReadCard
        ...
        <el-col :span="3">
          <LFormtTitle label="病种">
            <el-input v-model="form.odName"></el-input>
          </LFormtTitle>
        </el-col>
        <el-col :span="4">
    ...
    <div class="table-cont">
      ...
      >
        <el-table :data="tableData" border width="100%" height="100%">
          <el-table-column
            v-for="(item, index) in slowDiseaseColumns"
            :key="index"
            :prop="item.prop"
            :label="item.label"
...
import { slowDiseaseColumns } from "../common/slowDisease.js";
...
export default {
  ...
  methods: {
    ...
    async loadTable() {
      ...
      try {
        let res = await service.opspDiseFind(params);
        if (res.code == 1) {
    ...
    async cancelRegSure() {
      ...
      try {
        ...
        let res = await service.deleteOpspDise(params);
        if (res.code === 1) {
