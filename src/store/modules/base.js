...
// 患者信息存储
//  取数据：   ...mapGetters("base", ["receivePatientData"])
//  存数据：
//  ...mapActions({
...
const state = {
  PublicHealthPatientAddress:{},
  PublicHealthPatientData: {},
...
const mutations = {
  SET_PublicHealthAddress: (state, data) => {
    console.log("SET_PublicHealthAddress :***** ", data);
    state.PublicHealthPatientAddress = data;
  },
  SET_PublicHealthPatientData: (state, data) => {
    console.log("SET_PublicHealthPatientData :>> ", data);
    state.PublicHealthPatientData = data;
  },
  PATIENT_EMR_SETTING: (state, dataObj) => {
    state.patientEmr = dataObj;
  },
  ...
  RECEPTIONIST_SETTING: (state, dataObj) => {
    ...
    if (
      window.location.href.indexOf("cisOne/cisThree/record") > -1 &&
      state.receivePatientData.diagName === undefined
    ) {
...
const actions = {
  setPublicHealthAddress({ commit }, data) {
    commit("SET_PublicHealthAddress", data);
  },
  setPublicHealthPatientData({ commit }, data) {
    commit("SET_PublicHealthPatientData", data);
  },
  changePatientEmrData({ commit }, data) {
    commit("PATIENT_EMR_SETTING", data);
  },
...
const getters = {
  ...
  PublicHealthPatientAddress: state => state.PublicHealthPatientAddress,
  PublicHealthPatientData: state => state.PublicHealthPatientData,
  ...
  patientEmr: state => state.patientEmr
};
