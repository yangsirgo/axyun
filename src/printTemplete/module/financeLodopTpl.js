...
export let financeTpl = `LODOP.PRINT_INITA(0,0,700,382,"");
...
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(74,581,79,18,"stopTime");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
...
LODOP.ADD_PRINT_HTM(152,97,76,82,"drugOutpAmt");
LODOP.ADD_PRINT_HTM(152,176,77,77,"examOutp");
LODOP.ADD_PRINT_HTM(152,259,81,74,"examOutpAmt");
LODOP.ADD_PRINT_HTM(152,345,78,75,"treatOutp");
LODOP.ADD_PRINT_HTM(152,425,98,74,"treatOutpAmt");
LODOP.ADD_PRINT_HTM(152,524,78,74,"otherOutp");
LODOP.ADD_PRINT_HTM(152,605,85,73,"otherOutpAmt");
LODOP.ADD_PRINT_TEXT(228,96,73,21,"");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(228,258,86,19,"");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(229,424,91,20,"");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
...
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(101,253,153,20,"siSelfAccountMoney");
LODOP.ADD_PRINT_TEXT(120,253,151,20,"siFundPayMoney");
LODOP.ADD_PRINT_TEXT(101,416,147,20,"payTypeNm1");
LODOP.ADD_PRINT_TEXT(120,416,146,20,"payTypeNm2");
LODOP.ADD_PRINT_TEXT(18,347,320,20,"siAccountBalanceAfterPay");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(73,505,140,20,"chargeTime");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(18,168,178,20,"siAccountBalanceAfterPayBefore");
LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
LODOP.ADD_PRINT_TEXT(341,207,303,25,"tipNote");
LODOP.SET_PRINT_STYLEA(0,"FontSize",12);
LODOP.SET_PRINT_STYLEA(0,"Bold",1);`

const svgIcon = "<svg t='1617605864317' class='icon' viewBox='0 0 1024 1024' version='1.1' xmlns='http://www.w3.org/2000/svg' p-id='2227' width='16' height='16'><path d='M378.410667 850.450963C364.491852 850.450963 350.610963 845.293037 340.02963 834.939259L20.920889 523.529481C-0.279704 502.821926-0.279704 469.295407 20.920889 448.587852 42.121481 427.880296 76.48237 427.880296 97.682963 448.587852L378.410667 722.526815 925.75763 188.491852C946.958222 167.784296 981.319111 167.784296 1002.519704 188.491852 1023.720296 209.161481 1023.720296 242.688 1002.519704 263.395556L416.791704 834.939259C406.172444 845.293037 392.291556 850.450963 378.410667 850.450963L378.410667 850.450963Z' p-id='2228' fill='#1296db' stroke='#1296db' stroke-width='80'></path></svg>"
    ...
    if (key === 'finClassName') { // 费用项目
        return minimize`
            <div style='font-size: 13px;'>
                <% for(let i=0; i<data.length;i++){ %>
                    <div style='line-height: 1em;overflow:hidden;width:200px;height: 16px;'>
                        <%= data[i].finClassName %>
                    </div>
                <% } %>
            </div>
        ...
        return minimize`
            <div style='font-size: 14px;'>
                <% for(let i=0; i<data.length;i++){ %>
                    <div style='line-height: 1em;height: 16px;'>
                        <%= data[i].totalAmt %>
                    </div>
                <% } %>
            </div>
        ...
        `
    }else if(key === 'outpChargeDetailNewPOs') {
        return minimize`
...
export function createData(receiptContent, dayjs) {
    ...
    let obj = {
        ...
        drugOutp: "",
        drugOutpAmt: "",
        drugOutpFeeClassSumInfo: [],
        emrCode: "",
        examOutp: "",
        examOutpAmt: "",
        examOutpFeeClassSumInfo: "",
        freePayAmount: "",
        hospitalType: "",
        mouth: "",
        otherOutp: "",
        otherOutpAmt: "",
        otherOutpFeeClassSumInfo: "",
        outpChargeDetailNewPOs: "",
        ...
        payTypeNm1: "",//个人支付
        payTypeNm2: "",
        siAccountBalanceAfterPay: "",//支付后金额
        siAccountBalanceAfterPayBefore: "",//支付前金额
        chargeTime: "",//具体时间
        tipNote: "",// 挂号与门诊收费的提示信息
    };
    ...
    obj = {
        ...
        siSelfAccountMoney: receiptContent.siSelfAccountMoney ? "IC卡支付:" + receiptContent.siSelfAccountMoney : "",// IC卡支付
        siFundPayMoney: receiptContent.siFundPayMoney ? "统筹基金:" + receiptContent.siFundPayMoney : "",// 统筹基金
        siAccountBalanceAfterPay: receiptContent.siAccountBalanceAfterPay ? "医保结账后金额：" + receiptContent.siAccountBalanceAfterPay : "",//支付后金额
        reCheckUserName: receiptContent.regFeeFlag == "1" ? receiptContent.reCheckUserName : receiptContent.doctorName, // 如果是是门诊预结算，发票显示医生名称
        tipNote: receiptContent.regFeeFlag == "1" ? "挂号当天有效，请妥善保管发票" : "请妥善保管发票，遗失不补"
    };
    //支付前金额
    let num = Number(receiptContent.siAccountBalanceAfterPay || 0) + Number(receiptContent.siSelfAccountMoney || 0);
    obj.siAccountBalanceAfterPayBefore = num ? "医保结账前金额：" + num.toFixed(2) : "";
    // 判断性别
    if (receiptContent.patientGenderCode === "1") {
        obj.patientGender1 = obj.svgIcon
    } else if (receiptContent.patientGenderCode === "2") {
    ...
    if (receiptContent.clinicType === "1") {
        ...
        obj.stopTime =
            dayjs(receiptContent.stopTime).format("YYYY-MM-DD") || "";
    ...
    // 个人支付
    for(let i in obj.selfPayOutpTradeSettleAmtDetailPOS) {
        let item = obj.selfPayOutpTradeSettleAmtDetailPOS[i];
        if (i == 0) {
            obj.payTypeNm1 = `${item.payTypeName}:${item.payAmt}`
        } else if (i == 1) {
            obj.payTypeNm2 = `${item.payTypeName}:${item.payAmt}`
        }
    }
    console.log(obj);
    return obj;
...
function complier(data) { // 发票模板处理逻辑
    ...
    let drugOutpFeeClassSumInfo = data.drugOutpFeeClassSumInfo <= 3 ? data.drugOutpFeeClassSumInfo : data.drugOutpFeeClassSumInfo.splice(0,3);
    drugOutp = drugParse(drugOutpFeeClassSumInfo);
    let drugAmtParse = eval(compile(contentTpl('totalAmt')));
    drugOutpAmt = drugAmtParse(drugOutpFeeClassSumInfo);
    let examParse = eval(compile(contentTpl('finClassName')));
    let examOutpFeeClassSumInfo = data.examOutpFeeClassSumInfo <= 3 ? data.examOutpFeeClassSumInfo : data.examOutpFeeClassSumInfo.splice(0,3);
    examOutp = examParse(examOutpFeeClassSumInfo);
    let examAmtParse = eval(compile(contentTpl('totalAmt')));
    examOutpAmt = examAmtParse(examOutpFeeClassSumInfo);
    let treatParse = eval(compile(contentTpl('finClassName')));
    let treatOutpFeeClassSumInfo = data.treatOutpFeeClassSumInfo <= 3 ? data.treatOutpFeeClassSumInfo : data.treatOutpFeeClassSumInfo.splice(0,3);
    treatOutp = treatParse(treatOutpFeeClassSumInfo);
    let treatAmtParse = eval(compile(contentTpl('totalAmt')));
    treatOutpAmt = treatAmtParse(treatOutpFeeClassSumInfo);
    let otherParse = eval(compile(contentTpl('finClassName')));
    let otherOutpFeeClassSumInfo = data.otherOutpFeeClassSumInfo <= 3 ? data.otherOutpFeeClassSumInfo : data.otherOutpFeeClassSumInfo.splice(0,3);
    otherOutp = otherParse(otherOutpFeeClassSumInfo);
    let otherAmtParse = eval(compile(contentTpl('totalAmt')));
    otherOutpAmt = otherAmtParse(otherOutpFeeClassSumInfo);
    ...
    let chargeParse = eval(compile(contentTpl('outpChargeDetailNewPOs')));
    let outpChargeDetailNewPOs = data.outpChargeDetailNewPOs.length <= 4 ? data.outpChargeDetailNewPOs : data.outpChargeDetailNewPOs.splice(0,4);
    let outpChargeDetailNewPOsStr = chargeParse(outpChargeDetailNewPOs || []);
    return {
        svgIcon,
        drugOutp,
        drugOutpAmt,
